...
class ntfCell(LayerRNNCell):
  ...
  def build(self, inputs_shape):
    ...
    self._kernel_context = self.add_variable(
        ...
        shape=[self._num_units, 5 * self._n_seg])
  ...
  def call(self, inputs, state):
    ...
    unscaled_inputs = inputs#self._max_values*inputs
    ...
    def next_density(in_vars):
        with tf.name_scope("next_density"):
            ...
            T = tf.constant(1.0,name="T")
            seg_len = tf.constant(500.0,name="seg_len")
            lane_num = tf.constant(3.0,name="lane_num")
            flow0 = x0[0]
            flow1 = x1[0]
            r_in = c1[0]
            r_out = c1[1]
        rho =  current_density + tf.divide(T,seg_len*lane_num) * (flow0 - flow1 + r_in - r_out)
        return tf.clip_by_value(rho,0.0,2000.0)

    def velocity_fn(x):
        return tf.clip_by_value(tf.div(x[0],x[1]+1e-6),0.0,120.0)
    ...
    def next_velocity(in_vars):
        with tf.name_scope("next_velocity"):
            ...
            current_density = x1[1]
            seg_len = tf.constant(500.,name="seg_len_v")
            lane_num = tf.constant(3.0,name="lane_num_v")
            T = tf.constant(1.,name="T_v")
            tau = tf.constant(120.,name="tau")
            nu = tf.constant(35.,name="nu")
            kappa = tf.constant(13.,name="kappa")
            delta = tf.constant(1.4,name="delta")
            v_f = tf.constant(120.,name="v_f")
            a = c1[2]
            p_cr = c1[3]
            r_in = c1[0]

            stat_speed =  tf.clip_by_value(v_f * tf.exp( (-1/a) * (current_density*p_cr) **a ),0.0,200.0)
        ...
        return tf.clip_by_value(next_vel,0.0,120.0)
