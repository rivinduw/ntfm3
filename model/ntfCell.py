...
class ntfCell(LayerRNNCell):
  ...
  def call(self, inputs, state):
    ...
    #
    # each_seg = unscaled_inputs*tf.reduce_mean(next_states)/tf.reduce_mean(next_states)
    # each_seg = tf.Print(each_seg,[tf.shape(each_seg)],"each_seg")#[32 45 2]
    # each_seg = tf.reshape(each_seg,[32,90])*tf.reduce_mean(eq_vars)/tf.reduce_mean(eq_vars)
    # # each_seg = tf.Print(each_seg,[each_seg,tf.shape(each_seg)],"each_seg")
    # each_seg = array_ops.split(value=each_seg, num_or_size_splits=self._n_seg, axis=1)
    #
    #
